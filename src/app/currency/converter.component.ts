...
export class CurrencyConverter implements OnInit {
  ...
  // which this can grab from DB if needed
  ...
  //Require form to have a validation- in rate input box
  validateRateForm() {
  ...
  //Validate against invalid number which allow only number and 2 decimal places
  ...
  */
  exchangeRage(isValidForm: boolean) {
    ...
    this.exchangeRateApi.getExchangeRate(d.fromName).subscribe(data => {
